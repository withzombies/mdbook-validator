#!/bin/bash
#
# validate-osquery.sh - Host-based osquery JSON output validator.
#
# This script validates JSON output from osqueryi --json commands.
# It runs on the HOST (not in container) and uses jq for JSON parsing.
#
# Input: JSON via stdin (from osqueryi --json output)
# Environment:
