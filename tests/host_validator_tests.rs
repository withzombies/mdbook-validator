...
fn test_host_validator_runs_script() {
    ...
    let result = run_validator(ECHO_VALIDATOR, "{}", None, None).expect("validator should run");
...
fn test_host_validator_passes_json_stdin() {
    ...
    let result =
        run_validator(ECHO_VALIDATOR, json_input, None, None).expect("validator should run");
...
fn test_host_validator_captures_exit_code() {
    ...
    let result =
        run_validator(EXIT_CODE_VALIDATOR, "{}", None, None).expect("validator should run");
...
fn test_host_validator_nonexistent_script_returns_error_exit() {
    ...
    let result = run_validator("nonexistent_script_xyz_123.sh", "{}", None, None)
